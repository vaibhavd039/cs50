...
char* indexes(const char* path)
{
    char* arr1 = malloc(strlen(path) + strlen("index.html") + 1);
    strcpy(arr1,path);
    strcat(arr1,"index.html");
    
    char* arr2 = malloc(strlen(path) + strlen("index.php") + 1);
    strcpy(arr2,path);
    strcat(arr2, "index.php");
    
    if(access(arr2, F_OK) == 0)
    {
        return arr2;
    }
    else if(access(arr1,F_OK) == 0)
    {
        return arr1;
    }
    free(arr1);
...
const char* lookup(const char* path)
{
    ...
    else if(strcasecmp(value, ".gif") == 0)
    {
        return "image/gif";
    }
    else if(strcasecmp(value, ".ico") == 0)
    {
        return "image/x-icon";
    }
    else if(strcasecmp(value, ".jpg") == 0)
    {
        return "image/jpeg";
    }
    else if(strcasecmp(value, ".js") == 0)
    {
        return "text/javascript";
    }
    else if(strcasecmp(value, ".php") == 0)
    {
        return "text/x-php";
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    strcpy(line2,line);
    for(int i=0,n=strlen(line2); i<n; i++)
    {
    ...
    char* target_new = malloc(strlen(target)+1);
    strcpy(target_new,target);
    if(strrchr(target_new, '?') != NULL && strlen(strchr(target_new,'?')) != 1)
    {
        strtok(target_new,"\?");
        char* userQuery = strtok(NULL,"\0");
        strcpy(query,userQuery);
    }
    else
    {
        strcpy(query,"");
    ...
    char* arr = malloc(strlen(target)+1);
    strcpy(arr,target);
    if(strrchr(arr, '?')!=NULL)
    {
        char* absPathNew = strtok(arr,"\?");
        strcpy(abs_path,absPathNew);
    }
    else
    {
        char* absPathNew = strtok(arr,"\0");
        strcpy(abs_path,absPathNew);
    }
